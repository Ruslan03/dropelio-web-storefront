...
const BadgeRating = ({ sold, ratings }: { sold: number, ratings: number[] }) => {
   const t = useTranslations('ProductReview')

   const avgRating = ratings?.filter((rating) => rating > 0)
      .reduce((sum: number, rating: any) => sum + (5 / (Number(rating) || 0)), 0)
   ...
   return (
      <div className='flex items-center gap-3'>
         ...
         <div className='flex items-center'>
            ...
            {halfStar > 0 && Array.from(new Array(1)).map((_, i) => (
               <svg key={i} xmlns="http://www.w3.org/2000/svg" className="text-yellow-400 w-4 h-auto fill-current"
                  viewBox="0 0 16 16">
                  <path
            ...
            {Array.from(new Array(emptyStar)).map((_, i) => (
               <svg key={i} xmlns="http://www.w3.org/2000/svg" className="text-yellow-400 w-4 h-auto fill-current "
                  viewBox="0 0 16 16">
                  <path
